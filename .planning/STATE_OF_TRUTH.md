...
- **Playwright progression suite (live game, 11 min): 29 passed / 2 failed / 4 skipped** (the 4 skips are the retired `/breach` tests, by design). Both failures share one root: completing Reception via the QA bridge never lands `reception` in `completedRooms`, which also blocks the break-room-unlock test. Completion mechanics work elsewhere (hospital_entrance completion test passes), so it's Reception-specific — likely interaction drift (riley is at tile (10,3), the test targets (10,4); Phase 26 furniture may block a tile, or dialogue-completion handling drifted). Evidence: `tests/progression/room-completion.spec.ts:37`, `door-unlocks.spec.ts:71`.
...
4. **Verification debt.** Nothing from Phases 16–27 has had a live human playthrough — all feel-calibration was explicitly deferred to the user. Phase 19 (TD standalone) and 21 (sponsor capstone) have no SUMMARY/VERIFICATION docs at all, and they are precisely the two flows a sponsor will see first. The automated layer already shows one live crack: **2 of 31 progression tests fail today** (Reception completion never registers — §3 Build health has the diagnosis). Run 01/07 (playtest/fix loop) covers the automated half; the human half is still open.
