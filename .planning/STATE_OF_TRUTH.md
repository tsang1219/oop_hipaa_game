...
_2026-07-02 second pass (a twin Run-02 session that ran concurrently): independently re-verified via three parallel audits plus live execution — production build exercised, Playwright progression suite run against the live game, unit scripts run. Corrections and additions folded in below; see `fable-runs/RUN_REPORT-02.md` for the collision story._

_2026-07-02 post-merge update: the overnight batch is now merged to `main` (Run 07 fix loop, Run 04 proposal, this run). Run 07 fixed hotspots §6.1 and §6.2 and the two failing progression tests — sections below carry ✅ markers where reality moved. `RUN_REPORT-07.md` is the fix ledger._

---

## 1. The one-paragraph truth

PrivacyQuest is **one unified game at a single route (`/`)**: a 12-room hospital RPG (6 content rooms + 6 hallway connectors) with 26 named NPCs, three distinct encounters (BreachDefense tower defense, PHI Sorter Papers-Please desk, Breach Triage queue), a three-act soft narrative, a v2 localStorage save, a sponsor-demo path with certificate capstone, and a standalone tower-defense arcade mode reachable from the start menu. Its shipped public title — set in `client/index.html`, mentioned in no planning doc — is **"HIPAApocalypse"** ("a Health Is Other People game by Andrew Tsang"). **Phases 11–27 are all code-complete and shipped as of 2026-06-10.** `tsc` passes clean, the production build is verified clean, and after the Run-07 merge the Playwright progression suite is fully green (§3 Build health). Run 07's machine traversal also proved the game is **beatable end-to-end** (fresh save → all 26 NPCs → PRIVACY GUARDIAN win, zero console errors). What has NOT happened: any live *human* playthrough of the Phase 16–27 work (feel, not function), and any of the four identity-pivot workstreams from `IDENTITY_AUDIT.md` — though Run 05 has since left character-voice *drafts* (`CHARACTER_NINA_DRAFT.md`, `CHARACTER_RILEY_DRAFT.md`, `CAST.md`, `VOICE_AND_HUMOR.md`) awaiting a taste read.
...
**Save system** (`client/src/lib/saveData.ts`, 187 lines): `SaveDataV2` (15 declared fields), key `pq:save:v2`, `migrateV1toV2` collapses 29 legacy keys, write-before-delete, idempotent, has a real vitest suite (`saveData.test.ts`) — see §6 for why that suite never runs. **The declared shape is not the persisted shape**: `useGameState.ts:153-171` merges 8 additional fields into the same blob (`currentRoomId`, `currentAct`, `act1Complete`, `act2Complete`, `actFlags`, `decisions`, `encounterResults`, `unifiedScore`) via a `merged as SaveDataV2` cast — any save validation written from the interface alone would silently drop live game state (§6.2).
...
**Build health** (all exercised live 2026-07-02):
- `npx tsc --noEmit` → **0 errors**.
- `npm run build` → **clean**. One warning worth knowing: the client is a **single 2.24 MB JS chunk** (562 KB gzip — Phaser + React + all game code, no code-splitting). Server bundle `dist/index.js` 4.6 KB.
- **The `attached_assets` symlink survives the build**: `dist/public/attached_assets/` materializes as real files locally (and CI copies it explicitly before building), so the shipped game has its audio and sprites. Deployed-URL smoke test still pending (checklist §0).
- Unit scripts (run via `npx tsx`, not vitest — see §6.1): `tests/sorterData.test.mts` **188/188 pass**, `tests/breachTriageOverlay.test.mts` **26/26 pass**.
- **Playwright progression suite (live game, 11 min): 29 passed / 2 failed / 4 skipped** (the 4 skips are the retired `/breach` tests, by design). Both failures shared one root: Reception completion never landed in `completedRooms`. **→ Superseded: Run 07 round 5b (`3b99504`) root-caused it (stale hardcoded Riley coordinates in the specs + a qa-room loader race) and fixed it — suite is 31/0/4, re-confirmed on merged `main` 2026-07-02.**
- Zero TODO/FIXME/HACK markers in `client/src`.
...
| Playable characters | 3 (`you`, `alex`, `nikhil`) — blurbs carry inside jokes ("Brown Brad Pitt", "Lead Gardener"); review before ship | `characters.ts` |
...
| `MILESTONES.md` | HISTORICAL | Ledger stops at v1.1 — no v2.0/2.1/2.2/2.3 entries; portraits/furniture/floors listed "deferred to v2.x" → shipped (25/26); cites removed HubWorldScene; both it and ROADMAP link `milestones/v1.1-ROADMAP.md` + `v1.1-REQUIREMENTS.md` — **neither archive file exists** (only the v1.0 pair is on disk) |
...
| `codebase/STACK.md` | STALE | Claims routes `/`+`/privacy`+`/breach`, port **5000** (live default is 8080; 5000 is only the Replit env), and lists deleted pages/scenes (`HubWorldPage`, `PrivacyQuestPage`, `BreachDefensePage`, `HubWorldScene`, `HallwayHub`); dependency versions still roughly right |
| `codebase/CONVENTIONS.md` | MOSTLY CURRENT | Style/naming rules hold; but every example cites deleted pages + the fragmented v1 save, and room-size claims elsewhere in its era ("6×6 to 12×10") are wrong — every content room is **20×15** in `roomData.json` (ROOM_DESIGN_STANDARDS is the doc that's right) |
| `codebase/INTEGRATIONS.md` | MOSTLY CURRENT | Save/QA-params sections are current; its Phaser-config section still lists **4 scenes incl. HubWorldScene** (live: 3) and hub-door events |
| `codebase/TESTING.md` | MOSTLY CURRENT | Correct on `/breach` removal, port 8080, 7 rooms; its "34/41 passing, 0 failing" is superseded — live run 2026-07-02: 29 passed / **2 failed** / 4 skipped (§3) |
...
1. **Test-harness split brain.** ✅ **FIXED 2026-07-02 by Run 07 round 3 (`5715128`)** — `npm test` now runs vitest + the data scripts; Playwright `testMatch` scoped to `*.spec.ts`. Original finding, kept for history: three test dialects share overlapping globs with no coherent runner: 11 Playwright `.spec.ts` in `tests/`; 2 node strip-types `.test.mts` scripts (`sorterData`, `breachTriageOverlay`) with bare `assert()`; 1 real vitest file (`client/src/lib/saveData.test.ts`). **No npm script runs vitest at all** — the save-migration tests are unreachable. `npm run test:visual` (Playwright, `testDir: './tests'`) mis-collects the two `.mts` files (no `test()` calls). A bare `npx vitest` would collect the Playwright specs and break. `tsconfig.json` excludes `**/*.test.ts` and doesn't include `tests/`, so **zero test files are typechecked** by `npm run check`. The safety net everyone assumes exists, doesn't.
2. **Save-format fragility.** ✅ **FIXED 2026-07-02 by Run 07 round 3 (`5715128`)** — new `validateSave()` guards version + shape and preserves the extended fields exactly as the caution below demanded (5 new vitest cases, hostile-blob smoke clean). Original finding, kept for history: `loadSave` does `JSON.parse(...) as SaveDataV2` — the `version` field is **never checked on load**, no shape validation. Corrupt JSON safely falls back to `defaultSave`, but structurally-valid-wrong-shape JSON (a v1 blob, a hand-edited save, a future v3) is trusted blindly → latent `.length`/`.map` crash on the very first screen. One guard function fixes this — **but write it from the real persisted shape, not the interface**: `useGameState.ts:153-171` stores 8 extra fields (`currentAct`, `decisions`, `encounterResults`, …) in the same blob via an `as SaveDataV2` cast. A validator that strips unknown fields would wipe act progress and encounter results while "fixing" the save.
...
4. **Verification debt — automated half now closed, human half open.** Run 01 played everything read-only (117 screenshots), Run 07 fixed all 26 findings plus two self-found criticals, and its stop-condition traversal beat the game organically (all 26 NPCs, acts 1→3, PRIVACY GUARDIAN win, zero page errors). What remains is purely human: feel-calibration playthroughs for Phases 16–27, the demo path, and standalone TD (checklist §1) — plus tone sign-off on the copy Run 07 authored (checklist §4).
5. **Asset pipeline via symlink — verified working, one check left.** Zero assets live in `client/public/` directly — everything (243 audio, 40 images) resolves through a `client/public/attached_assets` **symlink** into the repo root. Second pass verified: local `npm run build` follows the symlink and materializes real files in `dist/public/attached_assets/`, and the GitHub-Pages workflow independently copies the folder before building. Remaining risk is only the deployed URL itself (base-path + Pages artifact) — one smoke test closes this.
6. **Three deploy targets configured, one of them broken.** (a) **GitHub Pages is the real path**: `.github/workflows/deploy.yml` auto-builds and deploys on **every push to `main`** with base `/oop_hipaa_game/` — the game is presumably already live at `tsang1219.github.io/oop_hipaa_game/` and every merge to main ships immediately, whether intended or not. (b) `.replit` deployment runs `node ./dist/index.cjs` — **the build produces `dist/index.js`; that run command can never work.** (c) `vercel.json` is a third, static-only config. Nobody has decided which is canonical; the broken and vestigial ones are traps.
7. **`window.location.reload()` as teardown.** Demo-exit, TD win/lose, and capstone-exit all "clean up" by full page reload. Works, but it's load-bearing jank: any future persistence-on-exit or analytics beacon will silently break, and it makes those flows untestable in a single Playwright context.
...
Minor but real: SPRITE-002 (NPC walk animations registered but never played — NPCs are statues), FEEL-001..008 polish backlog in DEBUG_LOG, dangling `sam_access` reference in the training framework. Second-pass additions: **46 of the codebase's 51 `eslint-disable`s are `react-hooks/rules-of-hooks`, all inside `PHISorterOverlay.tsx` (31) and `BreachTriageOverlay.tsx` (15)** — conditionally-called hooks, fragile under reorder; **two encounter engines coexist** (the legacy quiz-battle path — `GameContainer` → `BattleEncounterScreen` + 50 `gameData.json` scenes — plus the newer overlay encounters), and the TD encounter is the odd one out, **hardcoded by room-id at `ExplorationScene.ts:2109`** while sorter/triage are data-driven NPC `encounterTrigger`s; **7 stale git worktrees + branches from 2026-03-11** under `.claude/worktrees/` (each `[ahead 1, behind 274]`, full pre-unification codebase copies — search noise and branch-list clutter); vestigial `GEMINI_API_KEY` in `.env` and drizzle/postgres scaffolding with no consumer anywhere.
