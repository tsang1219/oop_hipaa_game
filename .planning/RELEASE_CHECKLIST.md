...
- [ ] **[YOU+AGENT] Cast identity pass (faces, colors, directory) + likeness pipeline.** Live-feedback finding 2026-07-02: the arcs are good but characters visually blend — 26 NPCs share 9 sheets, portraits are upscaled walk-frames, everything is flat white Press Start 2P. Fix: real per-NPC pixel portraits (revives deferred Phase 7), signature color per character, a Staff Directory screen (cast codex with arc state), main-cast sprite dedupe, and a photo→pixel-likeness pipeline + open character manifest so adding a real person = 2 PNGs + 1 data entry. Full brief: `fable-runs/08-cast-identity-pass.md`. Taste-gated: portrait style, faces, colors, any font change all need your eyes. **Progress: signature colors SHIPPED 2026-07-02** (26 persona-matched hues, dialogue + sorter + request modal; proof in `screenshots/run08/`) — portraits, directory, sprite dedupe, font, likeness pipeline still open.
